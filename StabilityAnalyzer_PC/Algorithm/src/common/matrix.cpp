...
long double MatrixDeterminant(const matrix<_Ty>& rhs)
{
    ...
    {
        MaxValue = static_cast<long double>(0.0);
        for(auto i = k; i < stRank; i ++)
        {
...
long double MatrixSymmetryRegularCholesky(matrix<_Ty>& rhs)
{
    ...
    if(iReValue < 2)
        return static_cast<long double>(0);		//rhs不是对称正定阵
                                            ...
                                            matrix<_Ty>& v, _Ty eps)
{
    ...
    for(i=1; i<=m; i++)
        for(auto j=1; j<=n; j++)	a(i-1,j-1)=0.0;
    m1=mm;
    it=60;
    while(1)
    {
