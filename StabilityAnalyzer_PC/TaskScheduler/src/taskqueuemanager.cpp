    ...
    , m_executionWorker(nullptr)
{
    ...
    // 设置调度定时器，暂时先设置500ms吧
    m_schedulerTimer->setInterval(2000); // 每2秒检查一次队列
...
void TaskQueueManager::addHighPriorityTask(const QString &deviceId, Task *task)
{
    ...
    qDebug() << "高优先级任务添加High priority task added - Device:" << deviceId
             << "任务名:" << task->taskName()
             << "队列size:" << m_highPriorityQueue.size();
...
void TaskQueueManager::waitForTaskCompletion(Task *task)
{
    ...
    qDebug() << "Waiting for sync task completion:" << task->taskName() << "Task ID:" << task->taskName();
