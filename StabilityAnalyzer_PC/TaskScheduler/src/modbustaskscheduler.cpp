                                          ...
                                          , const QString& remark)
{
    ...
    if (m_deviceConfigMap.contains(deviceId)) {
        ...
        if (task) {
            ...
            if(isSync){
                // 等待任务完成
                waitForTaskCompletion(task, result, 5000);
            }
                                          ...
                                          , int isSync, const QVector<quint16>& writeData)
{
    ...
    if (m_deviceConfigMap.contains(deviceId)) {
        ...
        if (task) {
            ...
            // 添加到轮询队列（低优先级队列）
            m_taskQueueManager->addPollingTask(deviceId, task);
            qDebug() << "Polling task added to polling queue:" << taskName
                     << "for device (slaveId):" << slaveId;
        } else {
            emit errorOccurred("Task not found: " + taskName + " for device (slaveId): " + slaveId);
        }
...
void ModbusTaskScheduler::updateConnectionStatistics()
{
    ...
    int totalCount = m_deviceConfigs.size();
...
bool ModbusTaskScheduler::waitForTaskCompletion(Task* task, QVector<quint16>& result, int timeoutMs)
{
    ...
    qDebug() << "Waiting for sync task completion:" << task->taskName();
    ...
    if (timeoutTimer.isActive()) {
        // 任务正常完成
        timeoutTimer.stop();
        qDebug() << "Sync task completed successfully:" << task->taskName();
        
        // 获取任务结果
        QVariant taskResult = task->result();
        if (taskResult.canConvert<QVector<quint16>>()) {
            result = taskResult.value<QVector<quint16>>();
            return true;
        } else {
            qWarning() << "Task result conversion failed for task:" << task->taskName();
            return false;
        }
    } else {
        // 任务超时
...
{
    qDebug() << "Task completed - Device:" << res.deviceId
             << "Task:" << res.taskName
...
{
    qDebug() << "Queue status changed - High priority:" << highPriorityCount
...
bool ModbusTaskScheduler::isPortConfigChanged(const SerialConfig &serialConfig) const
{
    ...
    // 比较关键配置参数
    if (cachedConfig.baudRate != serialConfig.baudRate ||
        cachedConfig.dataBits != serialConfig.dataBits ||
        cachedConfig.parity.toInt() != serialConfig.parity ||
