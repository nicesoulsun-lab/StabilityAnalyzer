...
Item {
    ...
    property int topMargin: chartTitle !== "" || chartSubtitle !== "" ? 42 : 16
    ...
    property string pendingDebugReason: ""
    ...
    function seriesPointCounts() {
        var counts = []
        for (var i = 0; i < seriesCount(); ++i) {
            var series = seriesAt(i)
            var points = series && series.points ? series.points : []
    ...
    function scheduleDebug(reason) {
        ...
        debugLogTimer.restart()
    }

    function logState(reason) {
        console.log("[CompareTrendChart][state]",
                    "tag=", debugName(),
                    "reason=", reason,
                    "visible=", visible,
                    "size=", width + "x" + height,
                    "plot=", plotArea.width + "x" + plotArea.height,
                    "series=", seriesCount(),
                    "pointCounts=", seriesPointCounts(),
                    "minX=", minXValue,
                    "maxX=", maxXValue,
                    "minY=", minYValue,
                    "maxY=", maxYValue)
    ...
    function findNearestDataPoint(plotPoint) {
        ...
        for (var seriesIndex = 0; seriesIndex < seriesList.length; ++seriesIndex) {
            ...
            for (var i = 0; i < points.length; ++i) {
                var candidate = toDataPoint(points[i])
                if (!candidate)
                    continue

                if (candidate.x < visibleMinX || candidate.x > visibleMaxX
                        || candidate.y < visibleMinY || candidate.y > visibleMaxY) {
                    continue
                }
    ...
    onSeriesListChanged: scheduleDebug("seriesListChanged")
    onVisibleChanged: scheduleDebug("visibleChanged")
    onWidthChanged: {
        if (width > 0)
            scheduleDebug("widthChanged")
    }
    onHeightChanged: {
        if (height > 0)
            scheduleDebug("heightChanged")
    }
    Component.onCompleted: {
        syncVisibleRange()
        scheduleDebug("completed")
    }
    ...
    Timer {
        ...
        interval: 0
        repeat: false
        onTriggered: root.logState(root.pendingDebugReason)
    }
    ...
    Text {
        visible: root.chartSubtitle !== ""
        anchors.right: parent.right
        anchors.rightMargin: 18
        anchors.top: parent.top
        anchors.topMargin: 12
        text: root.chartSubtitle
        font.pixelSize: 11
        font.family: "Microsoft YaHei"
        color: "#7A8CA5"
    }
        ...
        Item {
            ...
            Repeater {
                ...
                delegate: CurveItem {
                    ...
                    anchors.fill: parent
                    lineColor: seriesData && seriesData.color ? seriesData.color : "#2F7CF6"
                    ...
                    dataPoints: seriesData && seriesData.points ? seriesData.points : []
                }
            }
