...
#define LOG_TRACE_TIME  LoggerTimingHelper trace_loggerTimingHelper(cuteLoggerInstance(), Logger::Trace, __FILE__, __LINE__, Q_FUNC_INFO); trace_loggerTimingHelper.start
#define LOG_DEBUG_TIME  LoggerTimingHelper debug_loggerTimingHelper(cuteLoggerInstance(), Logger::Debug, __FILE__, __LINE__, Q_FUNC_INFO); debug_loggerTimingHelper.start
#define LOG_INFO_TIME   LoggerTimingHelper info_loggerTimingHelper(cuteLoggerInstance(), Logger::Info,  __FILE__, __LINE__, Q_FUNC_INFO); info_loggerTimingHelper.start

//#define LOG_TRACE_TIME  LoggerTimingHelper(cuteLoggerInstance(), Logger::Trace, __FILE__, __LINE__, Q_FUNC_INFO).start
//#define LOG_DEBUG_TIME  LoggerTimingHelper(cuteLoggerInstance(), Logger::Debug, __FILE__, __LINE__, Q_FUNC_INFO).start
//#define LOG_INFO_TIME   LoggerTimingHelper(cuteLoggerInstance(), Logger::Info,  __FILE__, __LINE__, Q_FUNC_INFO).start
...
public:
    ...
    void write(const QDateTime& timeStamp, LogLevel logLevel, const char* file, int line, const char* function, const char* category,
               const QString& message, LogPlace place=ConsoleFile);
    void write(LogLevel logLevel, const char* file, int line, const char* function, const char* category, const QString& message, LogPlace place=ConsoleFile);
    // <<a<<b这种输出方式时调用
...
private:
    void write(const QDateTime& timeStamp, LogLevel logLevel, const char* file, int line, const char* function, const char* category,
               const QString& message, LogPlace place, bool fromLocalInstance);
...
#  endif
    ...
    void write(const QString& msg,Logger::LogPlace place/* = Logger::ConsoleFile*/) const;
private:
    Logger* m_l;
...
#  endif
    ...
    void start(const QString& msg = QString(),Logger::LogPlace place = Logger::ConsoleFile);
    void start(Logger::TimingMode mode, const QString& msg = QString(),Logger::LogPlace place = Logger::ConsoleFile);
