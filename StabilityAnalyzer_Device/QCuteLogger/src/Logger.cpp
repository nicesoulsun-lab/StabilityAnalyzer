...
static void qtLoggerMessageHandler(QtMsgType type, const QMessageLogContext& context, const QString& msg)
{
    ...
    // Qt 原生日志（qDebug/qWarning/qInfo/...）默认也写入文件，便于统一排查。
    // ConsoleFile 会同时走控制台与文件 appender，满足“所有日志落盘”的需求。
    Logger::globalInstance()->write(level, context.file, context.line, context.function, isDefaultCategory ? 0 : context.category, msg,Logger::ConsoleFile);
}
...
void Logger::write(const QDateTime& timeStamp, LogLevel logLevel, const char* file, int line, const char* function, const char* category,
                   const QString& message,Logger::LogPlace place, bool fromLocalInstance)
{
    Q_D(Logger);
    //qDebug()<<"read to write data "<<logLevel<<message;
    ...
    if (!logCategory.isNull() && !isGlobalInstance)
        globalInstance()->write(timeStamp, logLevel, file, line, function, logCategory.toLatin1(), message,place, true);
 ...
 */
void Logger::write(const QDateTime& timeStamp, LogLevel logLevel, const char* file, int line, const char* function, const char* category,
                   const QString& message,Logger::LogPlace place)
{
    write(timeStamp, logLevel, file, line, function, category, message,place, /* fromLocalInstance = */ false);
}
...
void Logger::write(LogLevel logLevel, const char* file, int line, const char* function, const char* category,
                   const QString& message,Logger::LogPlace place)
{
    write(QDateTime::currentDateTime(), logLevel, file, line, function, category, message,place);
}
...
void LoggerTimingHelper::start(const QString& block, Logger::LogPlace place)
{
    m_block = block;
    m_place = place;
    m_time.start();
...
void LoggerTimingHelper::start(Logger::TimingMode mode, const QString& block,Logger::LogPlace place)
{
    m_timingMode = mode;
    m_block = block;
    m_place = place;
    m_time.start();
    //qDebug()<<"this is a start "<<m_logLevel;
}
...
LoggerTimingHelper::~LoggerTimingHelper()
{
    ...
    m_logger->write(m_logLevel, m_file, m_line, m_function, 0, message,m_place);
}
...
void CuteMessageLogger::write(const char* msg, ...) const
{
    ...
    va_list va;
    va_start(va, msg);
    //qDebug()<<"msg write* ... "<<QString().vsprintf(msg, va);
...
void CuteMessageLogger::write(const QString &msg, Logger::LogPlace place) const
{
    //qDebug()<<"-----";
    m_l->write(m_level, m_file, m_line, m_function, m_category, msg,place);
}
