...
bool ModbusClient::connect()
{
    ...
    if (!invoked) {
        qCritical() << "调用 ModbusWorker::doConnectDevice 失败";
        return false;
    }

    m_cachedConnected.store(success);
                                                          ...
                                                          int startAddr, int quantity, const QByteArray& data)
{
    ...
    case WriteMultipleRegisters:
        // data layout: startAddr(2) + quantity(2) + byteCount(1) + registerValues(quantity * 2)
        // Only the payload bytes after the 5-byte header are actual register values.
        if (data.size() >= 5 + quantity * 2) {
            QVector<quint16> values;
            for (int i = 0; i < quantity; i++) {
                const int valueOffset = 5 + i * 2;
                quint16 value = static_cast<quint8>(data[valueOffset]) << 8
                        | static_cast<quint8>(data[valueOffset + 1]);
                values.append(value);
            }
    ...
    case WriteMultipleCoils:
        // data layout: startAddr(2) + quantity(2) + byteCount(1) + coilBytes
        if (data.size() >= 5 + (quantity + 7) / 8) {
            QVector<bool> values;
            for (int i = 0; i < quantity; i++) {
                int byteIndex = 5 + (i / 8);
                int bitIndex = i % 8;
...
ModbusResult ModbusClient::executeSyncOperation(QSharedPointer<ModbusRequest> request)
{
    ...
    // qDebug() << "[ModbusClient][Sync] begin requestId=" << requestId
    //          << "slave=" << request->slaveId()
    //          << "function=" << request->functionCode()
    //          << "connected=" << m_cachedConnected.load()
    //          << "worker=" << m_worker
    ...
    const bool invoked = QMetaObject::invokeMethod(
        ...
        "enqueueRequest",
        Qt::QueuedConnection,
        Q_ARG(ModbusRequestPtr, request));
    // qDebug() << "[ModbusClient][Sync] enqueue invoke requestId=" << requestId
    //          << "invoked=" << invoked
    //          << "worker=" << m_worker;
    if (!invoked) {
        ModbusResult result;
        result.exception = SendError;
        result.errorString = "invoke enqueueRequest failed";
        {
            QMutexLocker locker(&m_syncMutex);
    ...
    // 等待响应
    // qDebug() << "[ModbusClient][Sync] eventLoop exec requestId=" << requestId;
    eventLoop->exec();
    // qDebug() << "[ModbusClient][Sync] eventLoop quit requestId=" << requestId;
    ...
    // qDebug() << "[ModbusClient][Sync] end requestId=" << requestId
...
void ModbusClient::executeAsyncOperation(QSharedPointer<ModbusRequest> request, const QString& tag)
{
    ...
    const bool invoked = QMetaObject::invokeMethod(
        m_worker, "enqueueRequest", Qt::QueuedConnection,
        Q_ARG(ModbusRequestPtr, request));
    // qDebug() << "[ModbusClient][Async] enqueue invoke requestId=" << requestId
    //          << "tag=" << tag
    //          << "invoked=" << invoked;
}
...
{
    //qDebug() << "[ModbusClient][RequestCompleted] requestId=" << requestId
    //         << "exception=" << result.exception
    //         << "valueCount=" << result.values.size()
    //         << "error=" << result.errorString;
    // 处理同步操作
...
void ModbusClient::onWorkerConnectionStatusChanged(bool connected)
{
    ...
    qDebug() << "[ModbusClient][ConnStatus] connected=" << connected
