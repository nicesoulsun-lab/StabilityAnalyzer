...
FrequencyAnalysis::FrequencyAnalysis(QObject *parent) : QObject(parent)
{

}

void FrequencyAnalysis::InputSource(QVector<float> &data, Filter &f)
{
    DealProcess(data,f);
}

void FrequencyAnalysis::DealProcess(QVector<float> &datain, Filter &fil)
{
    int Freq;
    Freq=fil.Freq;//采样频率
    int Scala;
    ...
    QVector<float> xFreqOut(Len / 2 + 1), yFreqSOut(Len / 2 + 1);
    for (int i = 0; i < Len / 2 + 1; i++) {
    ...
    OutputSource(xFreqOut,yFreqSOut);
}


void FrequencyAnalysis::OutputSource(QVector<float> &datax, QVector<float> &datay)
{
    //往绘图区传递数据
    for (int i = 0; i < datax.size(); i++) {
        m_outDatas.push_back(QPointF(datax[i],datay[i]));
    }
}

QVector<QPointF> FrequencyAnalysis::outDatas() const
{
    return m_outDatas;
}
