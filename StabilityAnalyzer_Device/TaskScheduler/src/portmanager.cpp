...
#ifdef Q_OS_LINUX
    // 兼容 Linux 上由 socat 创建的显式 PTY 路径，例如 /tmp/ttySA_A_HOST。
    if (!portExists && portName.startsWith('/')) {
        QFileInfo explicitPath(portName);
        if (explicitPath.exists()) {
            qInfo() << "Port accepted by explicit path:" << portName;
            portExists = true;
        }
    }
#endif
...
bool PortManager::connectPort(const QString &portName)
{
    ...
    qDebug() << "[PortManager][Connect] port=" << portName
             << "clientConnectResult=" << connected
             << "clientIsConnected=" << client->isConnected()
             << "deviceCount=" << portInfo.devices.size();
    if (!connected) {
    ...
    for (Device *device : portInfo.devices) {
        ...
        qDebug() << "[PortManager][Connect] deviceId=" << device->deviceId()
                 << "port=" << portName
                 << "deviceIsConnectedAfterSync=" << device->isConnected();
    }
